...
var TimelineComponent = function (_a) {
    var timelines = _a.timelines, onSelectEvent = _a.onSelectEvent, showMultipleTimelines = _a.showMultipleTimelines, onSelectTimeline = _a.onSelectTimeline, startYear = _a.startYear, endYear = _a.endYear, onYearRangeChange = _a.onYearRangeChange, onDeleteTimeline = _a.onDeleteTimeline, onEditTimeline = _a.onEditTimeline;
    ...
    var editEvent = function (updatedEvent) {
        ...
        var timeline = timelines.find(function (timeline) {
            return timeline.events.find(function (event) {
                return (event.year === selectedEvent.year &&
        ...
        var eventIndex = timeline.events.findIndex(function (event) {
            return event.year === selectedEvent.year &&
                event.title === selectedEvent.title &&
                event.description === selectedEvent.description;
        });
    ...
    var getTimelineIndexOnEvent = function (event) {
        return timelines.findIndex(function (timeline) {
            //has event that has the same year, title, and description
            return timeline.events.find(function (timelineEvent) {
                return (timelineEvent.year === event.year &&
    ...
    var timeline_ranges = react_1.useMemo(function () {
        var width_ofscreen = typeof window !== 'undefined' ? window.innerWidth : 0;
                        ...
                        react_1["default"].createElement("div", { className: "events relative w-full" }, timeline_range.events.map(function (event, eventIndex) {
                            ...
                            return (react_1["default"].createElement("div", { key: eventIndex, className: "event absolute cursor-pointer", onClick: function () { return selectEvent(event); }, style: {
                                ...
                                react_1["default"].createElement("div", { className: "absolute " + (isAbove ? 'bottom-full mb-2' : 'top-full mt-2') + " left-1/2 transform -translate-x-1/2", style: (_a = {},
                                        ...
                                        _a) },
                                    react_1["default"].createElement("span", { className: "event-year block text-center " + colors[getTimelineIndexOnEvent(event) % colors.length] + " text-white rounded px-2 py-1" }, event.year),
                                    react_1["default"].createElement("span", { className: "event-title block text-center mt-1" }, event.title)),
                                react_1["default"].createElement("div", { className: "w-4 h-4 " + colors[getTimelineIndexOnEvent(event) % colors.length] + " rounded-full border-2 border-white dark:border-gray-900" })));
                        }))),
                    react_1["default"].createElement("div", { className: "", style: {
                            height: height_offset + 50 + "px "
                        } }, ' '),
                    selectedEvent &&
                        timeline_range.events.find(function (event) { return event.year === selectedEvent.year; }) && (react_1["default"].createElement(react_1["default"].Fragment, null,
                        react_1["default"].createElement("div", { style: {
                                padding: '25px'
                            } },
                            react_1["default"].createElement(EventDetails_1["default"], { event: selectedEvent, editEvent: editEvent })))))));
        })));
};
